...
- **`context-manifest.yaml`**: documento decisões como `decisions[]` com link pro ADR — agente downstream não precisa reler o raciocínio inteiro.
- **MCP tools** (`fw mcp`): uso `get_task_details` e `get_agent_contract` pra contexto sob demanda.
