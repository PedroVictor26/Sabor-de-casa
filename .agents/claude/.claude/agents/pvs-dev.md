...
- **`context-manifest.yaml`**: leio decisões antes de começar, atualizo com `files_touched[]` e `decisions[]` ao terminar.
- **MCP tools** (`fw mcp`): uso `get_task_details` pra buscar spec kernel sem ler arquivo inteiro.
