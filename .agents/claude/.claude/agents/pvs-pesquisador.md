...
- **MCP tools** (`fw mcp`): uso `get_squad_status`, `get_agent_contract`, `run_doctor_checks` pra auditar estado de squads sem ler arquivos.
- **`fw consolidate`**: após pesquisa que gera aprendizado cross-projeto, consolido no `playbook.yaml`.
- **`fw doctor`**: verifico integridade do framework antes de emitir veredito sobre ferramentas que dependem dele.
