...
- **`fw doctor`**: rodo antes de qualquer veredito. Se tem WARN ou FAIL, reporto como CONCERNS.
- **MCP tools**: uso `get_task_details` e `get_agent_contract` pra buscar contexto sem ler arquivos inteiros.
- **`context-manifest.yaml`**: leio as decisões registradas antes de revisar — não peço contexto de novo.
- **Deploy pipeline**: em projeto prod, valido que o deploy-pipeline foi seguido (não deploy direto).
